...
function secretaryTools(pi: ExtensionAPI): void {
  ...
  const taskPacketSchema = z.object({ schemaVersion: z.literal(1), outcome: z.string().min(1).max(4096), boundaries: z.array(z.string().min(1).max(4096)).max(16), acceptance: z.array(z.string().min(1).max(4096)).max(16), openQuestions: z.array(z.string().min(1).max(4096)).max(16), evidence: z.array(z.string().min(1).max(4096)).max(16), issue_anchors: z.object({ platform_issue_id: z.string().min(1).max(128), source_issue_id: z.string().min(1).max(128) }).optional() });
  ...
  semantic("pisec_list_issues", "List issues", "issue.list", z.object({ state: z.enum(["open", "triaged", "remediation_planned", "remediating", "candidate_ready", "integrated", "verifying", "resolved"]).optional(), limit: z.number().int().min(1).max(1000).optional() }), "read", params => ({ ...(params.state ? { state: params.state } : {}), ...(params.limit ? { limit: params.limit } : {}) }));
  ...
  semantic("pisec_escalate_issue", "Escalate issue to Pisec", "issue.escalate", z.object({ source_issue_id: z.string().min(1).max(128), category: z.enum(["permission", "access", "lifecycle", "tooling", "other"]), severity: z.enum(["blocking", "degraded", "improvement"]), summary: z.string().min(1).max(1024), details: z.string().min(1).max(4096), requested_action: z.string().min(1).max(4096), evidence: z.array(z.any()).max(64) }), "exec", params => ({ sourceIssueId: params.source_issue_id, category: params.category, severity: params.severity, summary: params.summary, details: params.details, requestedAction: params.requested_action, evidence: params.evidence }));
  ...
  semantic("pisec_prepare_workstream", "Prepare Pisec workstream", "workstream.prepare", z.object({ title: z.string().min(1).max(512), purpose: z.string().min(1).max(4096), brief: z.string().min(1).max(4096), task_packet: taskPacketSchema, target_ref: z.string().min(1).max(512).optional(), source: importSourceSchema.optional(), implementation_model: z.string().min(1).max(256).optional(), execution_profile: z.literal("worker-default").optional(), work_mode: z.enum(["FAST", "RIP", "BUILD", "MAJOR"]).optional(), learning_overlay: z.enum(["OFF", "LIGHT", "DEEP"]).optional(), learning_seam: z.string().min(1).max(1024).optional(), decision_ids: z.array(z.string().min(1).max(128)).max(16).optional(), python_env: z.string().min(1).max(4096).optional(), remediation_issue_id: z.string().min(1).max(128).optional() }), "read", params => ({ title: params.title, purpose: params.purpose, brief: params.brief, taskPacket: params.task_packet.issue_anchors ? { schemaVersion: params.task_packet.schemaVersion, outcome: params.task_packet.outcome, boundaries: params.task_packet.boundaries, acceptance: params.task_packet.acceptance, openQuestions: params.task_packet.openQuestions, evidence: params.task_packet.evidence, issueAnchors: { platformIssueId: params.task_packet.issue_anchors.platform_issue_id, sourceIssueId: params.task_packet.issue_anchors.source_issue_id } } : params.task_packet, ...(params.target_ref ? { targetRef: params.target_ref } : {}), ...(params.source ? { source: params.source } : {}), ...(params.implementation_model ? { implementationModel: params.implementation_model } : {}), ...(params.execution_profile ? { executionProfile: params.execution_profile } : {}), ...(params.work_mode ? { workMode: params.work_mode } : {}), ...(params.learning_overlay ? { learningOverlay: params.learning_overlay } : {}), ...(params.learning_seam ? { learningSeam: params.learning_seam } : {}), ...(params.decision_ids ? { decisionIds: params.decision_ids } : {}), ...(params.python_env ? { pythonEnv: params.python_env } : {}), ...(params.remediation_issue_id ? { remediationIssueId: params.remediation_issue_id } : {}) }), value => rememberPreparedApproval(value, "workstream.create"));
  pi.registerTool({
...
function fleetTools(pi: ExtensionAPI): void {
  ...
  fleet("pisec_fleet_list_issues", "List issues", "fleet.issue.list", z.object({ project_id: projectId.optional(), state: z.enum(["open", "triaged", "remediation_planned", "remediating", "candidate_ready", "integrated", "verifying", "resolved"]).optional(), limit: z.number().int().min(1).max(1000).optional() }), "read", params => ({ ...(params.project_id ? { projectId: params.project_id } : {}), ...(params.state ? { state: params.state } : {}), ...(params.limit ? { limit: params.limit } : {}) }));
...
function workerTools(pi: ExtensionAPI): void {
  ...
  runtimeTool("pisec_inspect_attention", "Inspect attention", "Read one authorized attention record and its typed source, including state, next action, and the exact accepted completion contract for integration attention. Do not pass an integration source ID to a coordination inspector.", z.object({ attention_id: z.string().min(1).max(128) }), "attention.inspect", params => ({ attentionId: params.attention_id }));
  ...
  runtimeTool("pisec_submit_completion", "Submit completion", "Use as the sole final handoff after implementation and verification. Normally submit one immutable completion packet for the current worker commit; replaying the same packet is safe. If broker-authenticated integration attention reports accepted target drift, read source.accepted_completion_contract, submit one replacement completion packet after bounded reconciliation and reverification, and keep its criterion text, order, and passed status unchanged while providing current evidence. The broker creates the matching ready_review checkpoint atomically under the existing acceptance.", z.object({ completion: z.object({ acceptance: z.array(z.object({ criterion: z.string().min(1).max(4096), status: z.literal("passed"), evidence: z.array(z.string().min(1).max(4096)).min(1)})).min(1).max(32), verification: z.array(z.object({ command: z.string().min(1).max(4096), result: z.string().min(1).max(8192)})).min(1).max(32), source_commit: z.string().regex(/^(?:[0-9a-f]{40}|[0-9a-f]{64})$/), task_packet_sha256: z.string().regex(/^[0-9a-f]{64}$/), changed_surfaces: z.array(z.string().min(1).max(4096)).max(32), residual_risk: z.string().max(4096) }) }), "workstream.completion.submit", params => ({ completionPacket: { acceptance: params.completion.acceptance, verification: params.completion.verification, sourceCommit: params.completion.source_commit, taskPacketSha256: params.completion.task_packet_sha256, changedSurfaces: params.completion.changed_surfaces, residualRisk: params.completion.residual_risk } }));
