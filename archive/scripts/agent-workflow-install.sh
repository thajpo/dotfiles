...
HERDR_NVIM_REF="d1cc8ed5d53e7771471a6978454f3b1d566ef854"
...
herdr_plugin_state() {
  ...
  local repository="$3"
  local ref="$4"
  python3 - "$listing" "$plugin_id" "$repository" "$ref" <<'PY'
import json
import os
import re
import sys
...
if not isinstance(plugins, list):
    raise SystemExit("invalid")
repository = sys.argv[3].split("/")
if len(repository) != 2 or not all(repository):
    raise SystemExit("invalid")
expected_owner, expected_repo = repository
...
for plugin in plugins:
    if isinstance(plugin, dict) and plugin.get("plugin_id", plugin.get("id")) == sys.argv[2]:
        ...
        if (
            ...
            or source["owner"].casefold() != expected_owner.casefold()
        ...
        exact_commit = re.fullmatch(r"[0-9a-fA-F]{40}", expected_ref) is not None
        current = requested_ref == expected_ref and (
            not exact_commit
            or (isinstance(resolved_commit, str) and resolved_commit.casefold() == expected_ref.casefold())
        )
...
install_herdr_plugin() {
  ...
  state="$(herdr_plugin_state "$listing" "$plugin_id" "$repository" "$ref")" || die "Herdr returned invalid plugin metadata"
  case "$state" in
    absent|stale)
      ...
      state="$(herdr_plugin_state "$listing" "$plugin_id" "$repository" "$ref")" || die "Herdr returned invalid plugin metadata after installing $plugin_id"
      ;;
    current) ;;
    foreign) die "Herdr plugin $plugin_id is not the managed $repository installation" ;;
    invalid) die "Herdr plugin $plugin_id has invalid metadata or paths" ;;
    *) die "Herdr returned invalid plugin state for $plugin_id" ;;
  esac
  [[ "$state" == "current" ]] || die "Herdr plugin installation did not register pinned $plugin_id ref $ref"
...
chmod 0700 "$HOME/.local/lib/pisec"
...
transaction_capture_path "$HOME/.local/lib/pisec/stable-updater.json"
pisec_source_commit="$(git -C "$DOTFILES_DIR" rev-parse --verify HEAD)" || die "Pisec source checkout has no committed HEAD"
python3 "$DOTFILES_DIR/scripts/pisec-update.py" --install-updater-only --repo "$DOTFILES_DIR" --ref "$pisec_source_commit" || die "unable to install the committed Pisec stable updater"
