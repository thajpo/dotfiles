...
TOOL_DESCRIPTIONS = {
    ...
    "pisec_submit_completion": "Use as the sole final handoff after implementation and verification. Normally submit one immutable completion packet for the current worker commit; replaying the same packet is safe. If broker-authenticated integration attention reports accepted target drift, read source.accepted_completion_contract, submit one replacement completion packet after bounded reconciliation and reverification, and keep its criterion text, order, and passed status unchanged while providing current evidence. The broker creates the matching ready_review checkpoint atomically under the existing acceptance.",
    ...
    "pisec_inspect_attention": "Use after attention.list. Returns the authorized attention record and its typed source, including state, next action, and the exact accepted completion contract for integration attention; read-only. Do not pass an integration source ID to a coordination inspector.",
