...
class BrokerDispatcher:
    def __init__(
        ...
        *,
        ...
        self._startup_refresh_thread: threading.Thread | None = None
        self._startup_refresh_lock = threading.Lock()
    ...
    def startup_refresh_async(self, *, wait_seconds: float = 300.0) -> None:
        """Refresh stale approved runtimes after a broker restart.

        Herdr can restore a pane while the agent profile that was applied to
        it belongs to an older runtime surface.  ``startup_reconcile``
        intentionally leaves that state stopped so a normal reconcile cannot
        launch an unauthenticated or stale runtime.  A broker restart is the
        explicit recovery boundary: run the existing, journaled refresh engine
        in the background so the service becomes available immediately while
        stale idle runtimes are safely re-bound.
        """
        with self._startup_refresh_lock:
            if self._startup_refresh_thread is not None and self._startup_refresh_thread.is_alive():
                return
            self._startup_refresh_thread = threading.Thread(
                target=self._run_startup_refresh,
                args=(wait_seconds,),
                name="pisec-startup-refresh",
                daemon=True,
            )
            self._startup_refresh_thread.start()

    def _run_startup_refresh(self, wait_seconds: float) -> None:
        try:
            from .refresh import refresh_runtimes

            with self.store_factory() as store:
                result = refresh_runtimes(
                    store,
                    self.harness,
                    self.workspace,
            ...
            logger.info(
                "startup runtime refresh completed: upgraded=%d pending=%d failed=%d",
                len(result.get("upgraded", [])),
                len(result.get("pending", [])),
                len(result.get("failed", [])),
            )
        except BaseException:
            logger.exception("startup runtime refresh failed")
    ...
    def _secretary(self, store: PiStore, operation: str, project_id: str, secretary_workstream_id: str, payload: dict[str, Any]) -> Any:
        ...
        if operation == "workstream.prepare":
            _exact(payload, {"title", "purpose", "brief", "taskPacket", "idempotencyKey"}, {"targetRef", "source", "executionProfile", "pythonEnv", "workMode", "learningOverlay", "learningSeam", "decisionIds", "implementationModel", "remediationIssueId"})
