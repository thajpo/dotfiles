...
export function buildAsyncRunnerSteps(id: string, params: AsyncRunnerStepBuildParams): AsyncRunnerStepBuildResult {
	...
	const buildSeqStep = (s: SequentialStep, sessionFile?: string, behaviorCwd?: string, progressPrecreated = false, resolvedBehavior?: ResolvedStepBehavior, flatIndex?: number, parallelOutputNamespace?: { stepIndex: number; taskIndex?: number }) => {
		...
		const primaryModel = resolveEffectiveSubagentModel(
			...
			{ scope: ctx.modelScope, acceptanceRole: a.acceptanceRole },
		);
		...
		return {
			...
			modelCandidates: buildModelCandidates(primaryModel, a.fallbackModels, availableModels, ctx.currentModelProvider, { scope: ctx.modelScope, acceptanceRole: a.acceptanceRole }).map((candidate) =>
...
export function executeAsyncSingle(
	...
	const recoveryDescriptor: SteeringRecoveryDescriptor = {
		...
		...(agentConfig.acceptanceRole ? { acceptanceRole: agentConfig.acceptanceRole } : {}),
		...
		outputMode,
		// Persist the original public input. Resolved acceptance adds runtime-only
		// fields (explicit/inferredReason) that the recovery schema rejects.
				...
				steps: [
					{
						...
						model,
						thinking: resolveEffectiveThinking(model, effectiveThinking),
						modelCandidates: buildModelCandidates(primaryModel, agentConfig.fallbackModels, availableModels, ctx.currentModelProvider, { scope: ctx.modelScope, acceptanceRole: agentConfig.acceptanceRole }).map((candidate) =>
