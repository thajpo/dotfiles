  ...
  case "${2:-}" in
    list)
      ...
      if [[ -f "$config_root/chmarax.herdr-nvim/installed" ]]; then
        requested_ref="$(<"$config_root/chmarax.herdr-nvim/installed")"
        append_plugin "{\"plugin_id\":\"chmarax.herdr-nvim\",\"version\":\"0.2.0\",\"plugin_root\":\"$github_root/chmarax.herdr-nvim\",\"manifest_path\":\"$github_root/chmarax.herdr-nvim/herdr-plugin.toml\",\"source\":{\"kind\":\"github\",\"owner\":\"ChmaraX\",\"repo\":\"herdr-nvim\",\"requested_ref\":\"$requested_ref\",\"resolved_commit\":\"$requested_ref\"}}"
      fi
      if [[ -f "$config_root/persiyanov.reviewr/installed" ]]; then
        requested_ref="$(<"$config_root/persiyanov.reviewr/installed")"
        append_plugin "{\"plugin_id\":\"persiyanov.reviewr\",\"version\":\"0.32.1\",\"plugin_root\":\"$github_root/persiyanov.reviewr\",\"manifest_path\":\"$github_root/persiyanov.reviewr/herdr-plugin.toml\",\"source\":{\"kind\":\"github\",\"owner\":\"persiyanov\",\"repo\":\"herdr-reviewr\",\"requested_ref\":\"$requested_ref\",\"resolved_commit\":\"fixture-reviewr-commit\"}}"
      fi
    ...
    install)
      ...
      [[ "${4:-}" == "--ref" && -n "${5:-}" ]] || exit 2
      printf '%s\n' "$5" > "$config_root/$plugin_id/installed"
...
''')
    ...
    def test_full_install_links_stable_services_and_waits_for_health(self):
        ...
        self.assertIn("ExecStartPost=/usr/bin/systemd-run --user --quiet --collect --property=PartOf=pisec-broker.service /bin/sh -c 'for attempt in $(seq 1 30); do %h/.local/bin/pisec project refresh --all --wait-seconds 300 --json && exit 0; sleep 1; done; exit 1'", broker_unit)
    ...
    def test_full_install_replaces_stale_managed_herdr_plugin(self):
        config_root = self.home / ".config" / "herdr" / "plugins" / "config"
        github_root = self.home / ".config" / "herdr" / "plugins" / "github"
        nvim_state = config_root / "chmarax.herdr-nvim" / "installed"
        nvim_state.parent.mkdir(parents=True)
        nvim_state.write_text("40aadeab3cef3702ef5e05069181c7168084794f\n")
        nvim_manifest = github_root / "chmarax.herdr-nvim" / "herdr-plugin.toml"
        nvim_manifest.parent.mkdir(parents=True)
        nvim_manifest.touch()
        reviewr_state = config_root / "persiyanov.reviewr" / "installed"
        reviewr_state.parent.mkdir(parents=True)
        reviewr_state.write_text("v0.32.1\n")
        reviewr_manifest = github_root / "persiyanov.reviewr" / "herdr-plugin.toml"
        reviewr_manifest.parent.mkdir(parents=True)
        reviewr_manifest.touch()

        result = subprocess.run(
            [str(INSTALLER), "--collie-host", "pisec.example.ts.net", "--collie-trusted-user", "tester@example.invalid"],
            cwd=ROOT,
            env=self.env(),
            text=True,
            capture_output=True,
        ...
        self.assertEqual(result.returncode, 0, result.stderr + result.stdout)
        self.assertEqual(nvim_state.read_text().strip(), "d1cc8ed5d53e7771471a6978454f3b1d566ef854")
        self.assertEqual(
            (self.root / "herdr-plugin.log").read_text().splitlines(),
            ["chmarax.herdr-nvim", "herdr.collie"],
        )
