...
class ActivationCliTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.state_root = self.data / "state"
    ...
    def test_test_fixture_rejects_without_marker(self) -> None:
        self._stage()
        self.assertEqual(main(["--staged-root", str(self.stage), "--data-root", str(self.data), "--test-only-decision", "approve"]), 2)
    ...
    def test_test_fixture_rejects_outside_tmp(self) -> None:
        ...
        try:
            ...
            marker.chmod(0o600)
            os.environ["PI_ACTIVATE_TEST_FIXTURE"] = "1"
            self.assertEqual(main(["--staged-root", str(self.stage), "--data-root", str(outside / "data"), "--test-only-decision", "approve"]), 2)
        finally:
            os.environ.pop("PI_ACTIVATE_TEST_FIXTURE", None)
    ...
    def test_test_fixture_approve_activates_and_initializes_fresh_state(self) -> None:
        ...
        self._marker()
        try:
            result = main(["--staged-root", str(self.stage), "--data-root", str(self.data), "--test-only-decision", "approve"])
    ...
    def test_test_fixture_reject_does_not_activate(self) -> None:
        ...
        self._marker()
        try:
            result = main(["--staged-root", str(self.stage), "--data-root", str(self.data), "--test-only-decision", "reject"])
            self.assertEqual(result, 0)
    ...
    def test_missing_staged_root_fails(self) -> None:
        self._marker()
        try:
            self.assertEqual(main(["--staged-root", str(self.root / "missing"), "--data-root", str(self.data), "--test-only-decision", "approve"]), 2)
        finally:
            self._unmark()
    ...
    def test_activate_twice_preserves_rollback(self) -> None:
        ...
        self._marker()
        try:
            self.assertEqual(main(["--staged-root", str(self.stage), "--data-root", str(self.data), "--test-only-decision", "approve"]), 0)
            second = self.root / "stage2"
            from tests.system.staged_install import install
            install(second)
            self._marker()
            self.assertEqual(main(["--staged-root", str(second), "--data-root", str(self.data), "--test-only-decision", "approve"]), 0)
