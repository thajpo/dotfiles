...
from tests.test_greenfield_install import ROOT as SOURCE_ROOT, pack_test_pi_core
...
def _register_build(store: GreenfieldStore, root: Path) -> None:
...
class GreenfieldCoreTests(unittest.TestCase):
    ...
    def test_process_launcher_holds_and_releases_run(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            ...
            staged = stage(SOURCE_ROOT, root / "stage", pi_core_tarball=core)
            client = GreenfieldControllerClient(root / "state")
            registered = client.register_project(str(_repo(root, "launcher")), "launcher")
            with GreenfieldStore(root / "state") as store:
                register_staged_build(store, root / "stage")
            ...
            result = subprocess.run([str(launcher), "--state-root", str(root / "state"), "--conversation-id", conversation["conversation_id"], "--build-id", staged["buildId"], "--", "python3", "-c", code], cwd=str(root), stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, check=False, timeout=30)
