...
""")
            ...
            self.assertEqual(args[args.index("--tools") + 1],
                             "read,grep,find,ls,secretary_record_idea,secretary_create_workstream,secretary_open_workstream,secretary_list_workstreams,secretary_list_attention,secretary_acknowledge_attention,secretary_create_reviewer,secretary_land_reviewed,secretary_create_integration,secretary_cleanup_workstream")
...
""")
            ...
            self.assertEqual(result.returncode, 0, result.stderr)
            calls = [json.loads(line) for line in log.read_text().splitlines()]
            self.assertFalse(any(call[0] in {"send-keys", "split-window", "new-session"}
                                 for call in calls))
...
""")
            ...
            self.assertIn("exactly one projects window", result.stderr)
