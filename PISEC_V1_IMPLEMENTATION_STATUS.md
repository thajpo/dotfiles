...
- Phase 9 status: complete; README, installer/platform truth, stale CLI help, and the authenticated runtime-state boundary now match the v1 contract. Raw Herdr `done` remains presentation-only and is rejected by the Pisec runtime report/database path.
  ...
  - Current blocker: none.
- Phase 10 status: source acceptance reopened after live bootstrap exposed opaque archive, deployment-bytecode, and broker-start race defects; all three corrections are committed, and a new final source gate is pending before recovery/update proof.
  - Commit OID: none (live verification pending)
  - Checks: corrected bootstrap `23726af` contains all three fixes; the preceding isolated gate passed 208 Python tests/1 skip, verbose 208/1, and Bun 7/7; live final deployment is healthy after the startup race but has no LKG marker because the failed updater stopped before verification.
  - Current blocker: rerun complete source acceptance with bootstrap `23726af`, create a final source descendant, then restore verified bootstrap LKG and exercise manual recovery/update through the readiness-aware updater.
