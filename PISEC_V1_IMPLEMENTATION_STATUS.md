...
- Phase 9 status: complete; README, installer/platform truth, stale CLI help, and the authenticated runtime-state boundary now match the v1 contract. Raw Herdr `done` remains presentation-only and is rejected by the Pisec runtime report/database path.
  ...
  - Current blocker: none.
- Phase 10 status: corrected core and its final documentation/source descendant are committed; corrected live acceptance and the one final archive/reset remain.
  - Commit OID: `56b03a4f795b8eeee1c055114cc705c8427756f3` (`bootstrapV1Commit` candidate); `508a764fd45c8acc2ebf561fb4b0d72455112376` (`finalV1Commit` candidate).
  - Checks: fail-first reproduced the database-only mode-switch defect; public-dispatch movement coverage, Codex `0.150.0` pin/launcher coverage, provider-routing coverage, and focused adapter/doctor/fence/parity coverage passed (65 tests); the corrected Codex package-policy regression coverage passed (59 tests), including the exact Node and Codex entry-point launch exceptions; clean final-candidate acceptance passed (275 Python tests, 1 skipped; Bun 9/9; compileall, shell syntax, catalogue parity, Bun build, and `git diff --check`); independent static audit passed; unrelated dirty Neovim paths remain excluded. The bounded live Codex probe reached the configured gateway provider and was refused only for provider `Insufficient balance`.
  - Current blocker: complete corrected live acceptance when the configured gateway provider is available, then map all 51 scenarios, archive/reset once, replay exactly the nine permanent projects, prove the six-workspace Herdr topology, write owner-only external evidence, create the local `pisec-v1.0.0` tag, and perform final no-push verification.
