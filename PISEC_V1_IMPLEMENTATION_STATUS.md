...
- Phase 9 status: complete; README, installer/platform truth, stale CLI help, and the authenticated runtime-state boundary now match the v1 contract. Raw Herdr `done` remains presentation-only and is rejected by the Pisec runtime report/database path.
  ...
  - Current blocker: none.
- Phase 10 status: source acceptance complete through final4; live bootstrap/reset, fresh-schema verification, final update, and bootstrap last-known-good recovery predecessor are green; reviewed project re-registration is paused before any registration because one reviewed repository is missing.
  - Commit OID: `0c56f9494372215b7df24e60a08d1e1e0a31976a`
  - Checks: bootstrap `23726af` passed 209 Python tests/1 skip and Bun 7/7; final4 passed 210 Python tests/1 skip, verbose 210/1, Bun 7/7, build, compileall, shell syntax, catalogue, and diff checks; live current is final4 with exact v1 schema/bundle and compatible bootstrap LKG; no reviewed project was registered after the missing-path preflight.
  - Current blocker: `/tmp/pisec-live-acceptance-epoch3` from the reviewed runbook no longer exists, has no preserved Git worktree/object copy, and has no remote URL; restore that repository or authorize removing this stale temporary entry from the reviewed runbook before normal registration can continue.
