...
- Phase 7 status: complete; obsolete deployment/release/migration/launchd/probe surfaces and tests removed, current refusal/archive-reset coverage retained, active routing and archive warnings cleaned up, and obsolete active terminology removed.
  ...
  - Current blocker: none.
- Phase 8 status: complete; corrective bootstrap now preserves opaque pre-v1 archive entries, including stopped Unix sockets, while retaining fail-closed committed-bundle validation and archive/reset semantics.
  - Commit OID: `d91b6543d37aa3d9c271c1cf49ff22000caf70ac`
  - Checks: focused opaque-socket archive-manifest regression passed outside the sandbox; updater suite passed (11); compileall passed; `git diff --check` passed. The prior live reset failed closed before fresh-state initialization on an opaque socket and preserved the complete archive at `/home/j/.local/state/pisec.archive-20260825T160910Z`; source repair is committed and source acceptance is being rerun.
  - Current blocker: none.
- Phase 9 status: complete; README, installer/platform truth, stale CLI help, and the authenticated runtime-state boundary now match the v1 contract. Raw Herdr `done` remains presentation-only and is rejected by the Pisec runtime report/database path.
  ...
  - Current blocker: none.
- Phase 10 status: source acceptance reopened after the live bootstrap reset correctly rejected an opaque pre-v1 socket during archive-manifest hashing; corrected bootstrap and a new final source commit are pending the full acceptance gate before retry.
  - Commit OID: none (live verification pending)
  - Checks: corrected bootstrap `d91b654` passes the focused archive-manifest regression and updater suite; the complete source acceptance must be rerun with the corrected bootstrap lineage; the prior inventory/runbook remains owner-only and the failed reset archive is preserved.
  - Current blocker: source acceptance and corrected bootstrap archive/reset retry are pending; no fresh v1 state root or current deployment exists after the fail-closed reset.
