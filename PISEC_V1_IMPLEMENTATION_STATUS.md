...
- Phase 9 status: complete; README, installer/platform truth, stale CLI help, and the authenticated runtime-state boundary now match the v1 contract. Raw Herdr `done` remains presentation-only and is rejected by the Pisec runtime report/database path.
  ...
  - Current blocker: none.
- Phase 10 status: corrected Codex worker launcher source core is committed and green; the mandatory live Codex probe exposed and reproduced a launcher preflight defect, which is repaired; corrected-bootstrap replay remains historical partial state and final live acceptance remains.
  - Commit OID: `b41efa5a687bba9132cd318490b7b8e95b3d94bc` (bootstrapV1Commit; final documentation/status descendant pending)
  ...
  - Current blocker: commit the final documentation/status descendant and rerun the clean gate, install/deploy the corrected candidates, archive/reset the partial state containing the failed worker, then complete the 51 scenarios, external acceptance evidence, local v1 tag, and final verification.
