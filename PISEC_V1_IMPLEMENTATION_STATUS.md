...
- Phase 9 status: complete; README, installer/platform truth, stale CLI help, and the authenticated runtime-state boundary now match the v1 contract. Raw Herdr `done` remains presentation-only and is rejected by the Pisec runtime report/database path.
  ...
  - Current blocker: none.
- Phase 10 status: source acceptance reopened for and repaired a real fleet-transition usability defect; clean live reset and reviewed re-registration are in progress.
  - Commit OID: `ba4bde5` (source repair; final documentation/status descendant pending)
  - Checks: prior boundary-audit fail-first suite passed (5/5); lifecycle fail-first suite passed (2/2); focused repair/regression slice passed (129 tests, 1 skipped) before this repair and host-enabled project/adapter/scope slice passed (28 tests) after it; archive/reset passed with doctor/reconcile/refresh/semantic state green; exact reviewed legacy archive passed (15 roots, 60,634 files, 2,520,735,134 bytes, owner-only manifest); 14 existing repositories registered; canonical control, First Mate, and 9 intended-active Secretaries attested idle; jpo, vla-infra, and VLA Lens timeout retries reused completed operations; fleet transition fail-first exposed the missing harness identity and the source repair is committed; full clean Phase 10.1 gate and final candidate deployment remain pending.
  - Current blocker: the reviewed runbook contains one absent inactive temporary path (`/tmp/pisec-live-acceptance-epoch3`) and cannot be truthfully registered; the corrected candidate must be gated/deployed, fleet modes and permissions must complete, then 51 scenarios, recovery, external evidence, local v1 tag, and final verification remain.
