...
- Phase 9 status: complete; README, installer/platform truth, stale CLI help, and the authenticated runtime-state boundary now match the v1 contract. Raw Herdr `done` remains presentation-only and is rejected by the Pisec runtime report/database path.
  ...
  - Current blocker: none.
- Phase 10 status: corrected source core is committed and green; intermediate bootstrap replay is clean; final documentation descendant and corrected-bootstrap live cutover remain in progress.
  - Commit OID: `7282eb5c5d5478d2be15838ed1e7e0625f3e7887` (corrected source core; final documentation/status descendant pending)
  - Checks: boundary-audit fail-first suite passed (5/5); lifecycle fail-first suite passed (2/2); host-enabled focused repair/regression slice passed; archive/reset passed with doctor/reconcile/refresh/semantic state green; exact reviewed legacy archive passed (15 roots, 60,634 files, 2,520,735,134 bytes, owner-only manifest); 14 existing repositories registered and 9 intended-active Secretaries plus the canonical First Mate attested idle/bound; fleet refusal on the older bootstrap candidate was fail-first evidence; clean source gate passed with Python 265 tests and 1 skipped, Bun 8/8, compileall, shell syntax, catalogue parity, Bun build, and `git diff --check`.
  - Current blocker: final documentation/status truth must be committed and gated, then the corrected bootstrap must be reset/replayed; fleet modes, permissions, live worker/fleet/recovery paths, 51 scenarios, external evidence, local v1 tag, and final verification remain.
