...
- Phase 9 status: complete; README, installer/platform truth, stale CLI help, and the authenticated runtime-state boundary now match the v1 contract. Raw Herdr `done` remains presentation-only and is rejected by the Pisec runtime report/database path.
  ...
  - Current blocker: none.
- Phase 10 status: corrected source core is committed and green; final documentation/status descendant is being prepared before the final corrected-bootstrap replay and live acceptance.
  - Commit OID: `3eb53de5764c40744499812388e3bdcef93b5860` (corrected source core; documentation/status descendant pending)
  - Checks: final Luna source audit passed; host-enabled scope-parity suite passed (15/15); fence/activation suite passed (37, 1 skipped); clean corrected-core gate passed with Python 266 tests and 1 skipped, Bun 8/8, compileall, shell syntax, catalogue parity, Bun build, and `git diff --check`; exact reviewed legacy archive passed (15 roots, 60,634 files, 2,520,735,134 bytes, owner-only manifest).
  - Current blocker: commit and gate the final documentation/status descendant, then install the final updater and run the corrected-bootstrap reset/replay; fleet modes, permissions, worker/fleet workflows, recovery, 51 scenarios, external evidence, local v1 tag, and final verification remain.
