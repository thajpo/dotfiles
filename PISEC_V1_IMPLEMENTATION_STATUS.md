...
- Phase 9 status: complete; README, installer/platform truth, stale CLI help, and the authenticated runtime-state boundary now match the v1 contract. Raw Herdr `done` remains presentation-only and is rejected by the Pisec runtime report/database path.
  ...
  - Current blocker: none.
- Phase 10 status: corrected source core is committed and green; final corrected-bootstrap replay is at the inactive registration boundary; final documentation descendant and live acceptance remain in progress.
  - Commit OID: `a076175c7407eb3ca47396369dd3f4e5a208bcbe` (corrected source core; final documentation/status descendant pending)
  - Checks: boundary-audit fail-first suite passed (5/5); lifecycle fail-first suite passed (2/2); host-enabled focused scope/adapter slice passed (21/21); exact reviewed legacy archive passed (15 roots, 60,634 files, 2,520,735,134 bytes, owner-only manifest); corrected-bootstrap reset `deploy-090c19bfeaa24881a72deb5e17e61c71` passed green before this final fail-closed correction; clean corrected-core gate passed with Python 266 tests and 1 skipped, Bun 8/8, compileall, shell syntax, catalogue parity, Bun build, and `git diff --check`.
  - Current blocker: final documentation/status truth must be committed and gated, then the final updater must be installed and corrected core reset/replayed; fleet modes, permissions, worker/fleet workflows, recovery, 51 scenarios, external evidence, local v1 tag, and final verification remain.
