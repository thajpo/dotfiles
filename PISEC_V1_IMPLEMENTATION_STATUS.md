...
- Phase 9 status: complete; README, installer/platform truth, stale CLI help, and the authenticated runtime-state boundary now match the v1 contract. Raw Herdr `done` remains presentation-only and is rejected by the Pisec runtime report/database path.
  ...
  - Current blocker: none.
- Phase 10 status: corrected Codex launcher, activation-path, and bounded Herdr snapshot source core are committed and green; exact Codex pin is installed; the current replay exposed a partial Herdr-snapshot startup failure requiring the new candidate deployment/reset.
  - Commit OID: `7b1423d27f89ea36d17da174e974ff62219acb76` (bootstrapV1Commit candidate; final documentation/status descendant pending)
  - Checks: fail-first live launcher/config and large-snapshot evidence recorded; clean source gate passed with Python 268 tests and 1 skipped, Bun 9/9, compileall, shell syntax, catalogue parity, Bun build, and `git diff --check`; focused latest Herdr/runtime subset passed (51); Codex reports exact `0.147.0`; reset/replay began from the corrected 3c bootstrap and produced a partial state with no worker acceptance, integration, completion, or cleanup record.
  - Current blocker: commit the final documentation/status descendant, install/deploy 7b1423d, archive/reset the partial Herdr replay, then complete the 51 scenarios, external acceptance evidence, local v1 tag, and final verification.
