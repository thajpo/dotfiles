...
- Phase 9 status: complete; README, installer/platform truth, stale CLI help, and the authenticated runtime-state boundary now match the v1 contract. Raw Herdr `done` remains presentation-only and is rejected by the Pisec runtime report/database path.
  ...
  - Current blocker: none.
- Phase 10 status: source acceptance and controlled archive/reset are complete; bootstrap re-registration and live release acceptance remain.
  - Commit OID: `f2135c0784b541de7a50a1d2ce2a0a13b6cd0093` (finalV1Commit candidate; bootstrapV1Commit `fac003d6f69f2464a2ddc2c8d3d32d1cd781b44c`)
  - Checks: focused Phase 10 suite passed (21/21); adjacent adapter/Fence/runtime/install/updater cluster passed (69, 1 skipped); exact clean final gate passed with status clean, `git diff --check`, compileall, shell syntax, catalogue parity, Bun build, Python 273 tests (1 skipped), Bun 9/9, and exact Codex `0.147.0`; independent Luna audit passed; stable updater identifies final `f2135c0`; archive/reset of partial state succeeded via `deploy-79d4b348111e42e78733ef1c64b982fb` at bootstrap `fac003d`; archive manifest `pisec.archive-20260826T220944Z`; fresh DB has one `control_meta` row and all operational tables empty.
  - Current blocker: register/open the 15 reviewed repositories through normal commands, ensure First Mate and the four fleet modes/permissions, deploy final and prove recovery, execute all 51 scenarios and required whole-path integrations, write external acceptance evidence, create the local v1 tag, and perform final verification.
