...
- Phase 9 status: complete; README, installer/platform truth, stale CLI help, and the authenticated runtime-state boundary now match the v1 contract. Raw Herdr `done` remains presentation-only and is rejected by the Pisec runtime report/database path.
  ...
  - Current blocker: none.
- Phase 10 status: corrected worker-attestation source core is committed and green; corrected-bootstrap archive/reset, replay, fleet transitions, permissions, doctor, and reconcile are complete; final documentation/status descendant and live acceptance remain.
  - Commit OID: `773c034bb9b292b030df8a1662914af0ca62d6cb` (corrected source core; final documentation/status descendant pending)
  - Checks: fail-first worker non-UI startup test reproduced zero reports before repair; fresh Luna source audit passed; OMP suite passed (9/9); host-enabled scope-parity suite passed (15/15); fence/activation suite passed (37, 1 skipped); clean corrected-core gate passed with Python 266 tests and 1 skipped, compileall, shell syntax, catalogue parity, Bun build, and `git diff --check`; stable updater install, authorized archive/reset, 14 registrations, 9 opens, 1 First Mate, 4 fleet transitions, 9 permission replacements, doctor, and reconcile passed; archive manifest is owner-only and digest-matched.
  - Current blocker: gate the final documentation/status descendant, install it, run the ordinary update and recovery drill, then complete the 51 scenarios, external acceptance evidence, local v1 tag, and final verification.
