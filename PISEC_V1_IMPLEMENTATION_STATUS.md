...
- Phase 9 status: complete; README, installer/platform truth, stale CLI help, and the authenticated runtime-state boundary now match the v1 contract. Raw Herdr `done` remains presentation-only and is rejected by the Pisec runtime report/database path.
  ...
  - Current blocker: none.
- Phase 10 status: source acceptance is complete at the final documentation/status candidate; live deployment and acceptance remain.
  - Commit OID: `7298d6c644ef7ed67e031af42dc151b9f1517012` (finalV1Commit candidate; `64f4b4641c57f59940fd053e36c975e141d49636` is bootstrapV1Commit)
  - Checks: Codex launcher/state fail-first assertions failed before repair and passed after repair; focused Phase 10 suite passed (19/19); adjacent adapter/Fence/runtime/install/updater cluster passed (69, 1 skipped); exact final-candidate gate passed with clean status, `git diff --check`, compileall, shell syntax, catalogue parity, Bun build, Python 271 tests (1 skipped), Bun 9/9, and exact Codex `0.147.0`; direct exact Codex hook/provider control fired SessionStart and used loopback configuration; real-Fence disposable probe is environment-blocked by unavailable Linux bridge sockets.
  - Current blocker: install the stable updater from `7298d6c`, inventory and archive/reset the failed-worker state through the stable updater, complete the 51 scenarios, external acceptance evidence, local v1 tag, and final verification.
