...
- Phase 9 status: complete; README, installer/platform truth, stale CLI help, and the authenticated runtime-state boundary now match the v1 contract. Raw Herdr `done` remains presentation-only and is rejected by the Pisec runtime report/database path.
  ...
  - Current blocker: none.
- Phase 10 status: corrected Codex launcher and activation-path source core are committed and green; the mandatory live Codex probe exposed and reproduced both bounded defects, which are repaired; exact Codex pin is installed; final live acceptance remains.
  - Commit OID: `3c1034ff4334f605a8839690eaad0da36b4ebaa6` (bootstrapV1Commit candidate; final documentation/status descendant pending)
  - Checks: fail-first live launcher/config evidence recorded; focused Pisec cluster passed (111, 1 skipped); clean source gate passed with Python 267 tests and 1 skipped, Bun 9/9, compileall, shell syntax, catalogue parity, Bun build, and `git diff --check`; Codex reports exact `0.147.0`; prior reset/replay/registration/supervision/fleet/permission/doctor/reconcile/update/recovery checks passed; failed worker has no attestation, acceptance, integration, or completion records.
  - Current blocker: commit the final documentation/status descendant, install/deploy the corrected candidates, archive/reset the partial failed-worker state, then complete the 51 scenarios, external acceptance evidence, local v1 tag, and final verification.
