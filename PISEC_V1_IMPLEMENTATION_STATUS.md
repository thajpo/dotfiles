...
- Phase 9 status: complete; README, installer/platform truth, stale CLI help, and the authenticated runtime-state boundary now match the v1 contract. Raw Herdr `done` remains presentation-only and is rejected by the Pisec runtime report/database path.
  ...
  - Current blocker: none.
- Phase 10 status: refresh reservation lifecycle/CAS repair committed and independently audited; exact clean bootstrap source acceptance passed; final documentation/status candidate identity remains to be recorded; live cutover remains frozen.
  - Commit OID: `4127ebb6297043e0ec2e1f87ac6e949ac2e5c9a4` (corrected source core candidate)
  - Checks: prior boundary-audit fail-first suite passed (5/5); new lifecycle fail-first tests passed (2/2); focused repair/regression slice passed (129 tests, 1 skipped); exact clean Phase 10.1 gate at `4127ebb6297043e0ec2e1f87ac6e949ac2e5c9a4` passed: status clean, diff check, compileall, shell syntax, catalogue parity, Bun build, Python 264 tests with 1 skipped, and Bun 8/8; Luna read-only audit passed; no live-state mutation after the recorded partial state.
  - Current blocker: final documentation/status candidate identity and clean-gate rerun, then controlled recovery or authorized archive/reset, 51 scenarios, external evidence, local v1 tag, and final verification remain.
