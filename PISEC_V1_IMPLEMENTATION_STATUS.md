...
- Phase 9 status: complete; README, installer/platform truth, stale CLI help, and the authenticated runtime-state boundary now match the v1 contract. Raw Herdr `done` remains presentation-only and is rejected by the Pisec runtime report/database path.
  ...
  - Current blocker: none.
- Phase 10 status: source acceptance is complete at the corrected final documentation/status descendant; live release acceptance remains.
  ...
  - Checks: focused Phase 10 suite passed (21/21); adjacent adapter/Fence/runtime/install/updater cluster passed (69, 1 skipped); exact clean final gate passed with status clean, `git diff --check`, compileall, shell syntax, catalogue parity, Bun build, Python 273 tests (1 skipped), Bun 9/9, and exact Codex `0.147.0`; independent Luna audit passed after the sequence, event, request-ID, and public runtime-socket repairs.
  - Current blocker: inventory live Pisec/Pisec-worker state, install the final stable updater, archive/reset through its controlled path, complete bootstrap/final deployment and recovery proof, execute all 51 scenarios and required whole-path integrations, write external acceptance evidence, create the local v1 tag, and perform final verification.
