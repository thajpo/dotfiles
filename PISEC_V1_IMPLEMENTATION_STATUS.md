...
- Phase 9 status: complete; README, installer/platform truth, stale CLI help, and the authenticated runtime-state boundary now match the v1 contract. Raw Herdr `done` remains presentation-only and is rejected by the Pisec runtime report/database path.
  ...
  - Current blocker: none.
- Phase 10 status: corrected Codex worker launcher source core is committed and green; the mandatory live Codex probe exposed and reproduced a launcher preflight defect, which is repaired; the corrected final-candidate contract is now aligned across the authoritative plan and handoff; final live acceptance remains.
  - Commit OID: `b41efa5a687bba9132cd318490b7b8e95b3d94bc` (bootstrapV1Commit; final candidate is the clean tracked descendant)
  - Checks: fail-first live launcher exited 126 on the sealed `CODEX_HOME`; fail-first production launcher regression then passed after repair; Luna source audit passed; focused Pisec cluster passed (118, 1 skipped); clean bootstrap gate passed with Python 267 tests and 1 skipped, Bun 9/9, compileall, shell syntax, catalogue parity, Bun build, and `git diff --check`; prior corrected-bootstrap archive/reset, replay, registrations, supervision, fleet transitions, permissions, doctor, and reconcile passed; the live worker attempt correctly failed closed with no attestation, acceptance, integration, or completion records.
  - Current blocker: commit the authoritative plan/status correction and rerun the clean gate, install/deploy the corrected candidates, archive/reset the partial state containing the failed worker and inventoried stale worker refs, then complete the 51 scenarios, external acceptance evidence, local v1 tag, and final verification.
