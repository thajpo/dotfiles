# Pi Greenfield Program

## Program Status

- Status owner: this section only.
- Status as of: 2026-08-09, source `3338ba2fe181e6f27b815448077188583e095df0`, branch `master`.
- Worktree observation: dirty with 94 changed paths before P0 editing; those changes are intentional prior work and are not accepted by this document.
- Active work: P1-P10 gates pass against prior immutable generations; P11 release pipeline passes against single deterministic build `build_341120867dae530f75308795efbb1835` — all 18 HA actions covered by installed evidence, OpenCode protected, `bin/pi-activate` implemented (HA-012 `implemented-source`); P12 activation/rollback mechanics proven at `installed-passed`. The only remaining step before `release-passed` is the explicit controlling-TTY `bin/pi-activate` approval by the user on the real install. No release, activation, live-path, remote, tmux, integration, or rollback action was performed on production. |
- Activation: not authorized. OpenCode remains unchanged.
...
| P12 | `installed-passed` | Activation mechanics proven: `activation_approval.py` one-use approvals bound to exact build+cutover plan (schema v3, `activation_approvals` table); `greenfield_install.activate()` extended with launch lock (flock), bounded smoke (`bin/pi-control schema status`), protected-surface verification (OpenCode dirs + unrelated tmux sessions), and fresh-state init; P12 journey (`tests/system/fixtures/installed-p12.py`) staged A→activated A→journey→staged B→activated over A (A preserved as `.rollback.*`)→rollback restored A with state intact, OpenCode + tmux protected. HA-012 (`final-activation-approved`, tier `activation`) and HA-013 (`rollback-preserves-new-state`, tier `rollback`) evidence at `/tmp/pi-p11-release-evidence/`. `release-passed` requires the explicit controlling-TTY `bin/pi-activate` approval by the user on the real install — implementer stops here by agreement. |
...
| `pi/control-plane/GREENFIELD_CUTOVER_AND_ROLLBACK.md` | Atomic cutover and rollback transaction |
...
host conversational Pi / model / session / TUI process
  ...
  v
host controller
  |-- fresh greenfield store and CLI family
...
| Historical Pi state | Preserved external data with no greenfield authority and no read/adoption path |
...
| GF-STATE-001 | Greenfield starts fresh and never reads or adopts historical Pi state or chats. | State |
| GF-REACH-001 | The greenfield store/CLI family is canonical; retired runtime/controller families are unreachable from release actions. | State |
...
python3 -m unittest tests.system.test_greenfield_docs tests.system.test_action_manifest tests.system.test_evidence
...
| D-005 | One `PI_RUNTIME_MANIFEST`; no task-prefixed greenfield compatibility. |
| D-006 | Start fresh; preserve but never read or adopt old state and chats. |
| D-007 | Greenfield store/CLI is canonical; retired runtime/controller families are excluded from release reachability. |
...
The release graph begins at greenfield launchers, enters the greenfield CLI and
store family, and reaches only cataloged greenfield modules and packages. The
