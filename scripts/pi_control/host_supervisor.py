...
from .greenfield_install import RESOURCE_INVENTORY
...
_ROLE_OPERATIONS = {
    "secretary": {"scoped-read", "message.post", "message.list", "message.acknowledge", "message.reply", "dependency.disposition", "package-review.gate", "subagent.spawn"},
    "investigator": {"scoped-read", "package-review.record"},
    "reviewer": {"scoped-read", "package-review.gate"},
    "personal": {"writer-tool", "message.post", "message.list", "message.acknowledge", "message.reply", "command.request", "command.status", "dependency.inventory", "package.request", "package.status", "subagent.spawn"},
    "workstream": {"writer-tool", "message.post", "message.list", "message.acknowledge", "message.reply", "command.request", "command.status", "dependency.inventory", "package.request", "package.status", "subagent.spawn"},
    "integration": {"writer-tool", "message.post", "message.list", "message.acknowledge", "message.reply", "command.request", "command.status", "dependency.inventory", "package.request", "package.status", "subagent.spawn"},
}
_TOOL_RESOURCE = {
...
def _terminalize_temporary_role(store: Any, *, conversation: Mapping[str, Any], state: str, result: Mapping[str, Any]) -> None:
    role = conversation["role"]
    if role == "investigator":
        from .investigators import complete_conversation_investigation
        complete_conversation_investigation(store, conversation_id=conversation["conversation_id"], state=state, result=result)
    elif role == "reviewer":
...
def _authenticated_reader(store: Any, *, run_id: str, manifest_digest: str) -> tuple[ScopedProjectReader, str]:
    ...
    if run is None or run["desired_state"] != "running" or run["observed_state"] != "running" or run["authority"] != "host-read-only" or int(run["resource_version"]) != 1:
...
def _rpc(store: Any, run_id: str, manifest_digest: str, request: Mapping[str, Any], *, cancellation: threading.Event | None = None, child_context: Mapping[str, Any] | None = None) -> Any:
    ...
    if channel_operation == "subagent.spawn":
        if set(payload) != {"role", "task", "idempotencyKey"} or payload.get("role") not in {"investigator", "reviewer", "scout"} or not isinstance(payload.get("task"), str) or not isinstance(payload.get("idempotencyKey"), str):
    ...
    try:
        ...
        while process.poll() is None:
            ...
            def invoke() -> None:
                ...
                request_store = GreenfieldStore(store.state_root)
                try:
                    request_store.open()
                    outcome["result"] = _rpc(
        ...
        if selected["conversation"]["role"] in {"investigator", "reviewer"} and temporary_bound and not temporary_terminalized:
            ...
            _terminalize_temporary_role(store, conversation=selected["conversation"], state=state, result={"runId": prepared.run["run_id"], "error": type(error).__name__, "message": str(error)[:1024]})
            temporary_terminalized = True
        terminalized = True
        raise
    finally:
