...
def stage(
    ...
    *,
    ...
    manifest = create_build_manifest(
        stage_path,
        repository=source,
        metadata={"product": "pi-system", "artifactLayout": "pi-harness-v1", "freshState": True},
        test_outcomes={},
...
def activate(stage_root: os.PathLike[str] | str, data_root: os.PathLike[str] | str) -> dict[str, Any]:
    # Import before the rename: the stage path is still on sys.path, and the
    # module stays cached after the generation moves into the data root.
    ...
    try:
        ...
        try:
            ...
            if backup is not None:
                os.rename(backup, target)
            raise
        # Self-register the activated generation in its fresh state so the
        # daily surface can verify the build at launch. The activated root is
        # byte-exact for every manifest file; only activation.json and state/
        # are tolerated (verify_stage handles them).
        try:
            with PiStore(state_root) as store:
                register_staged_build(store, target)
        except BaseException as error:
            raise InstallError(f"activated generation registration failed: {error}") from error
