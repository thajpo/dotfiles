...
def _request(path: str) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise PiClientError("request JSON must be an object")
    return value
...
def main(argv: list[str] | None = None) -> int:
    ...
    client = PiControllerClient(args.state_root)
    try:
        if args.command == "schema":
            ...
            with client._store() as store:
                ...
                if operation == "read":
                    ...
                    raise PiClientError("unsupported scoped read operation")
        else:
            request = _request(args.request_json)
        ...
        _print(value)
        return 0
    except (ControlPlaneError, PiClientError, KeyError, ValueError, OSError, RuntimeError) as error:
