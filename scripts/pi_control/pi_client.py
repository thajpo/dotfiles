...
from .pi_workstreams import apply_workstream_proposal, approve_workstream_proposal, create_workstream, propose_workstream, retire_workstream
...
class PiControllerClient:
    ...
    def propose_workstream(self, **request: Any) -> dict[str, Any]:
        with self._store(mutate=True) as store:
            return propose_workstream(store, **request)

    def approve_workstream(self, **request: Any) -> dict[str, Any]:
        with self._store(mutate=True) as store:
            return approve_workstream_proposal(store, **request)

    def apply_workstream(self, **request: Any) -> dict[str, Any]:
        with self._store(mutate=True) as store:
            return apply_workstream_proposal(store, **request)
    ...
    def dispatch(self, operation: str, request: Mapping[str, Any] | None = None) -> dict[str, Any]:
        ...
        routes = {
            ...
            "workstream.propose": self.propose_workstream,
            "workstream.approve": self.approve_workstream,
            "workstream.apply": self.apply_workstream,
