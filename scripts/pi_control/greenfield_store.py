...
class GreenfieldStore:
    ...
    def __enter__(self) -> "GreenfieldStore":
        return self.open()
    ...
    def conn(self) -> sqlite3.Connection:
        if self.connection is None:
            raise RuntimeError("GreenfieldStore is not open")
        return self.connection

    def open(self) -> "GreenfieldStore":
    ...
    def _verify_schema(self) -> None:
        ...
        migration = self.conn.execute("SELECT name,source_sha256 FROM schema_migrations WHERE version=?", (GREENFIELD_SCHEMA_VERSION,)).fetchone()
        if migration is None or tuple(migration) != (GREENFIELD_MIGRATION_NAME, schema_digest()):
...
__all__ = ["GreenfieldStore", "default_state_root"]
