...
SPECS: Mapping[str, RequestSpec] = {
    ...
    "workstream.retire": _spec({"workstreamId": "workstream_id", "actorId": "actor_id", "expectedBranchRef": "expected_branch_ref", "expectedHeadOid": "expected_head_oid", "idempotencyKey": "idempotency_key"}, {"workstreamId", "actorId"}),
    "workstream.propose": _spec({"projectId": "project_id", "secretaryConversationId": "secretary_conversation_id", "secretaryRunId": "secretary_run_id", "title": "title", "purpose": "purpose", "targetRef": "target_ref", "knownOverlap": "known_overlap", "idempotencyKey": "idempotency_key"}, {"projectId", "secretaryConversationId", "secretaryRunId", "title", "purpose", "idempotencyKey"}),
    "workstream.approve": _spec({"messageId": "message_id", "actorId": "actor_id", "expiresAt": "expires_at"}, {"messageId", "actorId"}),
    "workstream.apply": _spec({"messageId": "message_id", "authorizationId": "authorization_id", "actorId": "actor_id"}, {"messageId", "authorizationId", "actorId"}),
