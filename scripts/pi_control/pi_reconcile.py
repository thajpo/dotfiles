...
def _run_observation(row: Any) -> dict[str, Any]:
    pid = row["owner_pid"]
    if pid is None or not row["owner_start_identity"]:
        return {"state": "unknown", "reason": "missing-owner-identity"}
    observation = observe_process(int(pid), expected_start_identity=str(row["owner_start_identity"]))
    if observation.state == "?":
        return {"state": "unknown", "reason": observation.error or "process-state-unavailable", "observation": observation.as_dict()}
    if observation.state in {"reused", "unknown"}:
        return {"state": "needs_attention", "reason": observation.error or observation.state, "observation": observation.as_dict()}
    if not observation.exists:
        return {"state": "needs_attention", "reason": "owner-process-gone", "observation": observation.as_dict()}
    return {"state": "running", "reason": "owner-identity-matches", "observation": observation.as_dict()}
...
def recover_lost_run(store: Any, *, run_id: str, actor_id: str) -> dict[str, Any]:
    ...
    if row["owner_pid"] is not None and row["owner_start_identity"]:
        observation = observe_process(int(row["owner_pid"]), expected_start_identity=str(row["owner_start_identity"]))
        if observation.state not in {"gone", "reused"} and (observation.exists or observation.state == "unknown"):
            raise ReconcileError("owner process is still present or unobservable")
    working_copy_id = row["working_copy_id"]
    if working_copy_id and row["authority"] == "writer":
        try:
            probe = WriterLock.acquire(store.state_root, working_copy_id, int(row["writer_epoch"] or 0) or 1)
        except WriterLockError as error:
            raise ReconcileError("writer lock is still held; recovery is unsafe") from error
        else:
            probe.close()
    now = utc_now()
    with store.transaction():
        ...
        append_event_in_transaction(store.conn, event_kind="run.recovered-lost", resource_type="run", resource_id=run_id, resource_version=int(row["resource_version"]), payload={"runId": run_id, "actorId": actor_id})
