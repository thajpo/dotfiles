...
GREENFIELD_SCHEMA_VERSION = 3
GREENFIELD_MIGRATION_NAME = "greenfield-epoch-3"
...
CREATE TABLE child_requests (
  ...
  parent_run_id TEXT NOT NULL REFERENCES runs(run_id),
  project_id TEXT NOT NULL REFERENCES projects(project_id),
  parent_working_copy_id TEXT NOT NULL REFERENCES working_copies(working_copy_id),
  semantic_role TEXT NOT NULL CHECK (semantic_role IN ('investigator','reviewer','scout')),
  runtime_role TEXT NOT NULL CHECK (runtime_role IN ('investigator','reviewer')),
  task TEXT NOT NULL,
  snapshot_id TEXT NOT NULL UNIQUE,
  snapshot_ref TEXT NOT NULL UNIQUE,
  snapshot_commit_oid TEXT NOT NULL,
  snapshot_tree_oid TEXT NOT NULL,
