...
function secretaryTools(pi: ExtensionAPI): void {
  ...
  semantic("pisec_install_release", "Install current Pisec runtime", "runtime.release.install", z.object({ wait_seconds: z.number().min(0).max(3600).optional() }), "exec", params => params.wait_seconds === undefined ? {} : { waitSeconds: params.wait_seconds });
