...
function workerTools(pi: ExtensionAPI): void {
  ...
  runtimeTool("pisec_inspect_attention", "Inspect attention", "Read one authorized attention record and its typed source, including state and next action when present. Do not pass an integration source ID to a coordination inspector.", z.object({ attention_id: z.string().min(1).max(128) }), "attention.inspect", params => ({ attentionId: params.attention_id }));
  ...
  runtimeTool("pisec_submit_completion", "Submit completion", "Use as the sole final handoff after implementation and verification. Normally submit one immutable completion packet for the current worker commit; replaying the same packet is safe. If broker-authenticated integration attention reports accepted target drift, submit one replacement completion packet after bounded reconciliation and reverification. Keep the accepted criterion text, order, and passed status unchanged; provide current evidence for the rebased commit. The broker creates the matching ready_review checkpoint atomically under the existing acceptance.", z.object({ completion: z.object({ acceptance: z.array(z.object({ criterion: z.string().min(1).max(4096), status: z.literal("passed"), evidence: z.array(z.string().min(1).max(4096)).min(1)})).min(1).max(32), verification: z.array(z.object({ command: z.string().min(1).max(4096), result: z.string().min(1).max(8192)})).min(1).max(32), source_commit: z.string().regex(/^(?:[0-9a-f]{40}|[0-9a-f]{64})$/), task_packet_sha256: z.string().regex(/^[0-9a-f]{64}$/), changed_surfaces: z.array(z.string().min(1).max(4096)).max(32), residual_risk: z.string().max(4096) }) }), "workstream.completion.submit", params => ({ completionPacket: { acceptance: params.completion.acceptance, verification: params.completion.verification, sourceCommit: params.completion.source_commit, taskPacketSha256: params.completion.task_packet_sha256, changedSurfaces: params.completion.changed_surfaces, residualRisk: params.completion.residual_risk } }));
